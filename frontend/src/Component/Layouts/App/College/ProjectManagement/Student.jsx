...
const Student = () => {
  ...
  const [mainTabs] = useState([
    ...
    { id: 'admission', name: 'Admission List', count: 0 },
    { id: 'zero_period', name: 'Zero Period', count: 0 },
    { id: 'batch_freeze', name: 'Batch Freeze List', count: 0 },
    { id: 'dropout', name: 'Dropout List', count: 0 }
  ]);
  ...
  const handleMainTabClick = (tabIndex, tabId) => {
    setActiveMainTab(tabIndex);
    setCurrentPage(1);

    // Filter data based on the selected main tab
    filterByMainTab(tabId);
  };

  const filterByMainTab = (tabId) => {
    let filteredData = [...allProfilesData];

    switch (tabId) {
      case 'all':
        filteredData = allProfilesData;
        break;
      case 'admission':
        // Filter for students in admission process
        filteredData = allProfilesData.filter(profile =>
          profile.admissionStatus === 'confirmed' ||
          profile._leadStatus?.title?.toLowerCase().includes('admission') ||
          profile.isAdmitted === true
        );
        break;
      case 'zero_period':
        // Filter for students in zero period
        filteredData = allProfilesData.filter(profile =>
          profile.zeroPeriod === true ||
          profile._leadStatus?.title?.toLowerCase().includes('zero') ||
          profile.batchStatus === 'zero_period'
        );
        break;
      case 'batch_freeze':
        // Filter for students with batch freeze
        filteredData = allProfilesData.filter(profile =>
          profile.batchFreeze === true ||
          profile._leadStatus?.title?.toLowerCase().includes('freeze') ||
          profile.batchStatus === 'frozen'
        );
        break;
      case 'dropout':
        // Filter for dropout students
        filteredData = allProfilesData.filter(profile =>
          profile.isDropout === true ||
          profile._leadStatus?.title?.toLowerCase().includes('dropout') ||
          profile._leadStatus?.title?.toLowerCase().includes('drop out') ||
          profile.studentStatus === 'dropout'
        );
        break;
      default:
        filteredData = allProfilesData;
  ...
  useEffect(() => {
    if (allProfilesData.length > 0) {
      // Reapply current filter when data changes
      const currentTab = mainTabs[activeMainTab];
                      filterByMainTab(currentTab.id);
    }
  }, [allProfilesData, activeMainTab]);

  // Update main tab counts
  const updateMainTabCounts = () => {
    const counts = {
      all: allProfilesData.length,
      admission: allProfilesData.filter(profile =>
        profile.admissionStatus === 'confirmed' ||
        profile._leadStatus?.title?.toLowerCase().includes('admission') ||
        profile.isAdmitted === true
      ).length,
      zero_period: allProfilesData.filter(profile =>
        profile.zeroPeriod === true ||
        profile._leadStatus?.title?.toLowerCase().includes('zero') ||
        profile.batchStatus === 'zero_period'
      ).length,
      batch_freeze: allProfilesData.filter(profile =>
        profile.batchFreeze === true ||
        profile._leadStatus?.title?.toLowerCase().includes('freeze') ||
        profile.batchStatus === 'frozen'
      ).length,
      dropout: allProfilesData.filter(profile =>
        profile.isDropout === true ||
        profile._leadStatus?.title?.toLowerCase().includes('dropout') ||
        profile._leadStatus?.title?.toLowerCase().includes('drop out') ||
        profile.studentStatus === 'dropout'
            ...
            <div className="container-fluid py-2 " >
              <div className="row align-items-center">
                <div className="col-md-6 d-md-block d-sm-none">
                  <div className="d-flex align-items-center">
                    <h4 className="fw-bold text-dark mb-0 me-3">Student</h4>
                ...
                <div className="card-body p-3">
                  <div className="d-flex flex-wrap gap-2 align-items-center">
                    ...
                    {updateMainTabCounts().map((tab, index) => (
                      <div key={tab.id} className="d-flex align-items-center gap-1">
                        <button
                          className={`btn btn-sm ${activeMainTab === index ? 'btn-primary' : 'btn-outline-secondary'} position-relative main-tab-btn`}
                          onClick={() => handleMainTabClick(index, tab.id)}
                          style={{
                            minWidth: '140px',
                        ...
                        >
                          <div className="d-flex flex-column align-items-center">
                            ...
                            <span className={`small ${activeMainTab === index ? 'text-white' : 'text-muted'}`} style={{ fontSize: '0.7rem' }}>
                              ({tab.count} Students)
                            </span>
                          </div>

                          {/* Add icon for each tab */}
                          <div className="tab-icon position-absolute" style={{ top: '5px', right: '8px' }}>
                            {tab.id === 'all' && <i className="fas fa-users" style={{ fontSize: '0.7rem' }}></i>}
                            {tab.id === 'admission' && <i className="fas fa-graduation-cap" style={{ fontSize: '0.7rem' }}></i>}
                            {tab.id === 'zero_period' && <i className="fas fa-hourglass-start" style={{ fontSize: '0.7rem' }}></i>}
                            {tab.id === 'batch_freeze' && <i className="fas fa-snowflake" style={{ fontSize: '0.7rem' }}></i>}
                            {tab.id === 'dropout' && <i className="fas fa-user-times" style={{ fontSize: '0.7rem' }}></i>}
                          </div>
                        </button>
                      </div>
                    ))}

{/* // Add this useEffect to update when data changes */}
