...
const emailComposeEditorConfig = {
    toolbar: [
        'heading', '|',
...
const EmailTemplate = () => {
    ...
    const replaceVariablesInText = (text) => {
        ...
        if (editForm.variables && Array.isArray(editForm.variables)) {
            editForm.variables.forEach(variable => {
                ...
                const value = variable.value || placeholder;
    ...
    const openCreateModal = () => {
        const modalElement = document.getElementById('createEmailTemplateModal');
        if (modalElement) {
    ...
    // Add New Email Template (full-page view) form state
    const [emailCreateForm, setEmailCreateForm] = useState({
        templateName: '',
        subject: '',
    ...
    const [showEmailVariableDropdown, setShowEmailVariableDropdown] = useState(false);
    const emailVariableDropdownRef = useRef(null);
    const emailComposeEditorRef = useRef(null); // CKEditor instance - to update content when variable is selected
    const emailVariablesSectionRef = useRef(null); // scroll into view when variables are added
    ...
    const [showEmailVisibleForDropdown, setShowEmailVisibleForDropdown] = useState(false);
    ...
    // Handle clicking outside email create variable dropdown
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (emailVariableDropdownRef.current && !emailVariableDropdownRef.current.contains(event.target)) {
                setShowEmailVariableDropdown(false);
            }
            if (!event.target.closest('.email-visible-for-dropdown')) {
                setShowEmailVisibleForDropdown(false);
            }
        };
        if (showEmailVariableDropdown || showEmailVisibleForDropdown) {
            document.addEventListener('mousedown', handleClickOutside);
        }
        return () => document.removeEventListener('mousedown', handleClickOutside);
    }, [showEmailVariableDropdown, showEmailVisibleForDropdown]);

    // When variables are added to email create form, scroll "Variables in your message" section into view
    useEffect(() => {
        if (emailCreateForm.variables?.length > 0 && emailVariablesSectionRef.current) {
            emailVariablesSectionRef.current.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        }
    }, [emailCreateForm.variables?.length]);
    ...
    // Toggle visible for counselor in email create form
    const toggleEmailVisibleFor = (counselor) => {
        const current = emailCreateForm.visibleFor || [];
        const updated = current.includes(counselor)
            ? current.filter(item => item !== counselor)
            : [...current, counselor];
        setEmailCreateForm({ ...emailCreateForm, visibleFor: updated });
    };

    const renderAddEmailTemplate = () => (
        <div className="container-fluid p-4" style={{ backgroundColor: '#f8f9fa', minHeight: '100vh' }}>
            {/* Header: Back + Title + Save As Draft */}
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div className="d-flex align-items-center">
                    <button
                        type="button"
                ...
                <h1 className="mb-0 fs-4 fw-bold" style={{ color: '#212529' }}>Add New Email Template</h1>
                <button
                    type="button"
                    className="btn btn-outline-secondary"
                    style={{ borderRadius: '8px' }}
                    onClick={() => {
                        // Save as draft - same as publish but mark draft if you have API support
                        setNotification({ type: 'info', message: 'Draft saved.', show: true });
                ...
                <div className="card-body p-4">
                    ...
                    <div className="mb-4">
                        ...
                        <input
                            ...
                            value={emailCreateForm.templateName}
                            onChange={(e) => setEmailCreateForm({ ...emailCreateForm, templateName: e.target.value })}
                    ...
                    <div className="mb-4">
                        ...
                        <input
                            ...
                            className="form-control"
                            placeholder="Subject"
                            value={emailCreateForm.subject}
                            onChange={(e) => setEmailCreateForm({ ...emailCreateForm, subject: e.target.value })}
                            style={{ borderRadius: '8px', border: '1px solid #ced4da' }}
                        />
                    </div>

                    {/* Variable - same predefined variables as used in template */}
                    <div className="mb-4 position-relative" ref={emailVariableDropdownRef}>
                        <label className="form-label fw-medium" style={{ fontSize: '14px' }}>Variable</label>
                        <div
                            ...
                            onClick={() => setShowEmailVariableDropdown(!showEmailVariableDropdown)}
                        >
                            <span className="text-muted">
                                {emailCreateForm.variables?.length ? `${emailCreateForm.variables.length} variable(s) selected` : 'Select Variable'}
                            </span>
                            <span className="text-muted">▼</span>
                        </div>
                        {showEmailVariableDropdown && (
                            <div
                                    ...
                                    <button
                                        ...
                                        onClick={() => {
                                            setEmailCreateForm((prev) => {
                                                ...
                                                const newBodyText = (prev.bodyText || '') + ` {{${variable.placeholder}}}`;
                                                // CKEditor only uses `data` on mount; update editor content so variable appears immediately
                                                queueMicrotask(() => {
                                                    if (emailComposeEditorRef.current) {
                                                        // CKEditor only uses `data` on mount; update editor content so variable appears immediately
                                                        if (emailComposeEditorRef.current) {
                                                            emailComposeEditorRef.current.setData(newBodyText);
                                                        }
                                                        emailComposeEditorRef.current.setData(newBodyText);
                                                    }
                                                });
                    ...
                    {/* Visible For */}
                    <div className="mb-4 email-visible-for-dropdown">
                        <label className="form-label fw-medium" style={{ fontSize: '14px' }}>Visible For</label>
                        <div
                            ...
                            onClick={() => setShowEmailVisibleForDropdown(!showEmailVisibleForDropdown)}
                        >
                            {(emailCreateForm.visibleFor || []).map((item) => (
                                <span
                                    key={item}
                                    className="badge d-inline-flex align-items-center gap-1"
                                    style={{ backgroundColor: '#e9ecef', color: '#495057', fontSize: '12px' }}
                                >
                                    {item}
                                    <span
                                        role="button"
                                        tabIndex={0}
                                        onClick={(e) => { e.stopPropagation(); toggleEmailVisibleFor(item); }}
                                        onKeyDown={(e) => e.key === 'Enter' && toggleEmailVisibleFor(item)}
                        ...
                        {showEmailVisibleForDropdown && (
                            <div
                                className="border rounded shadow-sm bg-white mt-1"
                                style={{ position: 'absolute', zIndex: 1040, minWidth: '200px' }}
                            >
                                {availableCounselors.filter(c => !(emailCreateForm.visibleFor || []).includes(c)).map((counselor) => (
                                    <button
                                        key={counselor}
                                        type="button"
                                        ...
                                        onClick={() => {
                                            setEmailCreateForm({
                                                ...emailCreateForm,
                                                visibleFor: [...(emailCreateForm.visibleFor || []), counselor]
                                            });
                                        }}
                                    >
                                        {counselor}
                    ...
                    <div className="mb-4">
                        <label className="form-label fw-medium" style={{ fontSize: '14px' }}>
                            Compose Email Template <span className="text-danger">*</span>
                        </label>
                        <div
                        ...
                        >
                            <CKEditor
                                ...
                                data={emailCreateForm.bodyText || ''}
                                config={emailComposeEditorConfig}
                                onReady={(editor) => {
                                    emailComposeEditorRef.current = editor;
                                }}
                                onChange={(event, editor) => {
                                    const data = editor.getData();
                                    setEmailCreateForm({ ...emailCreateForm, bodyText: data });
                                }}
                            />
                        </div>
                        <small className="text-muted d-block mt-2">Variables like {"{{name}}"}, {"{{email}}"} will be replaced when sending. Add them from the Variable dropdown above.</small>

                        {/* Variables in your message - same as in create template modal */}
                        {emailCreateForm.variables && emailCreateForm.variables.length > 0 && (
                            <div ref={emailVariablesSectionRef} className="mt-3">
                                <h6 className="fw-bold mb-2" style={{ color: '#495057', fontSize: '14px' }}>Variables in your message:</h6>
                                {emailCreateForm.variables.map((variable) => (
                                    <div key={variable.id} className="mb-3">
                                        <div className="d-flex align-items-center mb-2">
                                            <span
                                                className="badge me-2"
                                                style={{ backgroundColor: '#e3f2fd', color: '#1976d2', fontSize: '12px', padding: '4px 8px', borderRadius: '12px' }}
                                            >
                                                {variable.placeholder}
                                            </span>
                                            <span className="fw-medium" style={{ fontSize: '12px' }}>{variable.displayName || variable.placeholder} Value:</span>
                                        </div>
                                        <input
                                            type="text"
                                            ...
                                            onChange={(e) => {
                                                const updated = emailCreateForm.variables.map(v =>
                                                    v.id === variable.id ? { ...v, value: e.target.value } : v
                                                );
                                                setEmailCreateForm({ ...emailCreateForm, variables: updated });
                                            }}
                                            style={{
                    ...
                    <div className="mb-4">
                        ...
                        <button
                            type="button"
                            className="btn btn-outline-secondary"
                            style={{ borderRadius: '8px', borderColor: '#ff6b35', color: '#ff6b35' }}
                            onClick={() => document.getElementById('email-template-attachment').click()}
                        >
                            Browse
                        </button>
                        <input
                            id="email-template-attachment"
                            type="file"
                            className="d-none"
                            multiple
                            onChange={(e) => {
                                ...
                                setEmailCreateForm({
                                    ...emailCreateForm,
                                    attachments: [...(emailCreateForm.attachments || []), ...files]
                                });
                            }}
                        />
                        {emailCreateForm.attachments?.length > 0 && (
                            <span className="ms-2 text-muted small">{emailCreateForm.attachments.length} file(s) selected</span>
                        )}
                    </div>
                    ...
                    <div className="d-flex justify-content-end gap-2 pt-2">
                        ...
                        <button
                            ...
                            style={{ borderRadius: '8px', backgroundColor: '#ffb088' }}
                            onClick={() => setNotification({ type: 'info', message: 'Publish feature – connect to your API.', show: true })}
                        >
                            PUBLISH
                        </button>
                        <button
                            type="button"
                            className="btn text-white"
                            style={{ borderRadius: '8px', backgroundColor: '#ff6b35' }}
                            onClick={() => setNotification({ type: 'info', message: 'Test Template – connect to your API.', show: true })}
                        >
                            TEST TEMPLATE
                        </button>
                    </div>
        ...
        `}
            ...
            <div className="d-flex justify-content-between align-items-center mb-4">
                ...
                <div className="d-flex align-items-center">
                    ...
                    <span className="fs-4 me-3" style={{ color: '#ff6b35' }}>Email Templates⚡</span>
            ...
            <div className="modal fade" id="createEmailTemplateModal" data-bs-backdrop="static" data-bs-keyboard="false" tabIndex="-1" aria-labelledby="createEmailModalLabel" aria-hidden="true">
                ...
                <div className="modal-dialog modal-xl modal-dialog-scrollable">
                    ...
                    <div className="modal-content" style={{ borderRadius: '12px', border: 'none' }}>
                        ...
                        <div className="modal-header" style={{ borderBottom: '1px solid #e9ecef', padding: '1.5rem' }}>
                            ...
                            <h1 className="modal-title fs-4 fw-bold" id="createEmailModalLabel">

                                {isCloneMode ? 'Clone Email Template' : 'Create Email Template'}
...
#createEmailTemplateModal , #templatePreviewModal{
        ...
        return (
            <>
                {notification.show && (
                    ...
                    >
                        <div style={{ flex: 1, marginRight: '10px' }}>{notification.message}</div>
                        <button
