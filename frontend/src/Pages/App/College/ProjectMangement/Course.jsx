    ...
    <div className="container py-4 vt-page vt-screen-enter">
      ...
      <div className="vt-head">
        ...
        <div className="vt-head-actions">
          {onBackToCenters && (
            <button type="button" className="vt-back" onClick={onBackToCenters} title="Back to Centers">
        ...
        <div className="vt-tabs">
          {['Active Courses', 'Inactive Courses', 'All Courses'].map(tab => (
            <button
              type="button"
              key={tab}
              className={activeCourseTab === tab ? 'is-active' : ''}
      ...
      <div className="table-responsive">
        <table className="vt-table">
          ...
          <tbody>
            {filteredCourses.map(course => (
              <tr key={course._id} className="vt-row" title="Open batches" onClick={() => handleCourseClick(course)}>
                <td>
                ...
                <td className="vt-desc">{course.trainers?.length ? course.trainers.map(t => t.name).join(', ') : '—'}</td>
                <td>{course.sectors?.length ? course.sectors[0].name : '—'}</td>
                <td>
                  <span className={`vt-pill ${course.status === 'active' ? 'is-active' : ''}`}>{course.status}</span>
                </td>
                <td onClick={(e) => e.stopPropagation()}>
                  <div className="vt-actions">
                    <button
                      ...
                      onClick={() => {
                        setSelectedCourse(course);
                        setSelectedTrainers(course.trainers?.map(t => t._id) || []);
        ...
        <div className="vt-empty">
          ...
          <p>{selectedCenter ? `No courses found for ${selectedCenter.name}` : 'Try another filter or search.'}</p>
        </div>
      )}
      </div>
