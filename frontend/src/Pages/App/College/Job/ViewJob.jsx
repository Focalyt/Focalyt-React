...
function ViewJob() {
  ...
  const needsStream = QUALIFICATIONS_WITH_STREAM.includes(
    qualificationList.find((q) => q._id === form._qualification)?.name
  );
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        setIndustryList(industryRes.data?.industry || industryRes.data || []);
        setQualificationList(qualificationRes.data?.qualification || qualificationRes.data || []);
        setSubQualificationList(subQualificationRes.data?.subQualification || subQualificationRes.data || []);
        setStateList(stateRes.data?.state || stateRes.data || []);
        setCoinsRequired(coinsRes.data || { contactcoins: 0 });
        setTechSkillsList(techSkillsRes.data?.techskills || techSkillsRes.data || []);
        setNonTechSkillsList(nonTechSkillsRes.data?.nontechskills || nonTechSkillsRes.data || []);
      } catch (err) {
                  ...
                  <div className="row">
                    ...
                    <div className="col-xl-3 mb-1">
                      ...
                      <select
                        ...
                        value={form.benifits}
                        onChange={(e) =>
                          updateField('benifits', Array.from(e.target.selectedOptions, (o) => o.value))
                        }
                      >
