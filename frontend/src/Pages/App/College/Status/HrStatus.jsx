...
const handleMoveLeft = async (statusId, currentIndex) => {
  ...
  try {
    ...
    axios.put(`${backendUrl}/college/statusHr/reorder`, {
      statusOrder
    }, {
...
const handleMoveRight = async (statusId, currentIndex) => {
    ...
    const handleSave = async (statusId, substatusId) => {
      if (title.trim()) {
        if (isSubstatus) {
          if (editMode) {
            // Edit substatus
            const response = await axios.put(`${backendUrl}/college/statusHr/${statusId}/substatus/${substatusId}`, {
            ...
            // Add substatus
            const response = await axios.post(`${backendUrl}/college/statusHr/${statusId}/substatus`, {
              title, description, hasRemarks, hasFollowup, hasAttachment
            }, { headers: { 'x-auth': token } });
          ...
          if (editMode && statusId) {
            // Edit status
            const response = await axios.put(`${backendUrl}/college/statusHr/edit/${statusId}`, {
            ...
            // Add status
            const response = await axios.post(`${backendUrl}/college/statusHr/add`, {
              title, description,milestone
            }, { headers: { 'x-auth': token } });
  ...
  const fetchStatus = async () => {
    try {
      ...
      const response = await axios.get(`${backendUrl}/college/statusHr`, {
        headers: { 'x-auth': token }
      });
      ...
      console.error('Error fetching HR statuses:', error);
      alert('Failed to fetch HR statuses');
    }
  };
  ...
  const handleDrop = async (e, targetIndex) => {
    ...
    try {
      ...
      const response = await axios.put(`${backendUrl}/college/statusHr/reorder`, {
        statusOrder
      }, {
        headers: { 'x-auth': token }
      });
  ...
  const handleConfirmDelete = async () => {
    if (isDeletingSubstatus) {
      ...
      try {
        ...
        const response = await axios.delete(`${backendUrl}/college/statusHr/deleteSubStatus/${deletingStatusId}/substatus/${deletingSubstatusId}`, {
          headers: { 'x-auth': token }
        });
        ...
        alert(error.response?.data?.message || 'Error occurred while deleting sub status');
      }
    } else {
      try {
        const backendUrl = process.env.REACT_APP_MIPIE_BACKEND_URL;
        ...
        const response = await axios.delete(`${backendUrl}/college/statusHr/delete/${deletingStatusId}`, {
          headers: { 'x-auth': token }
        });
        ...
        alert(error.response?.data?.message || 'Error occurred while deleting status');
      }
    }
