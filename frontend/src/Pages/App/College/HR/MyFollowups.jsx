...
const formatLeadOwner = (leadOwner) => {
  if (!leadOwner || leadOwner === 'N/A') return 'N/A';
  if (Array.isArray(leadOwner)) {
    const names = leadOwner
      .map((owner) => (typeof owner === 'object' ? (owner.name || owner.email || '') : String(owner || '')))
      .filter(Boolean);
    return names.length ? names.join(', ') : 'N/A';
  }
  if (typeof leadOwner === 'object') return leadOwner.name || leadOwner.email || 'N/A';
...
const getHrNextActionDateValue = (details, profile) => {
  const dates = [
    details?.followup?.followupDate,
    details?.nextCallFollowup?.followupDate,
    details?.nextVisitFollowup?.followupDate,
...
const MyFollowups = () => {
  ...
  const handleMarkCompleteFollowup = async (profile) => {
    ...
    try {
      ...
      const leadId = profile.leadId;

      const response = await axios.post(`${backendUrl}/college/hr/leads/${leadId}/followup/${profile._id}/complete`, {
      ...
      if (response.data.success) {
        ...
        await fetchFollowupCounts();
      } else {
        alert(response.data.message || 'Failed to mark followup as complete');
      }
  ...
  const handleUpdateFollowup = async (e) => {
    ...
    try {
      ...
      const leadId = selectedProfile.leadId || selectedProfile.appliedCourseId;
      const data = {
        type: selectedProfile.followUpType || 'Call',
        followupDate: followupDateTime ? followupDateTime.toISOString() : null,
        remarks: remarks?.trim() || selectedProfile.remarks || 'Followup updated',
      };
      ...
      // HR followups live on CareerApplication, not AppliedCourses / B2cFollowup
      const response = await axios.post(
        `${backendUrl}/college/hr/leads/${leadId}/followup`,
        data,
        {
          headers: {
            'x-auth': token,
  ...
  const fetchProfileData = async (filters = filterData, page = currentPage) => {
    ...
    try {
      ...
      const response = await axios.get(`${backendUrl}/college/hr/my-followups?${queryParams}`, {
        headers: {
          'x-auth': token,
        },
      });
  ...
  const fetchLeadDetails = async (profile, profileIndex) => {
    ...
    try {
      ...
      const leadId = profile.leadId || profile.appliedCourseId;
      console.log('profile', profile)
      const response = await axios.get(`${backendUrl}/college/hr/leads/${leadId}`, {
        headers: { 'x-auth': token }
      });
      ...
      if (response.data.success && response.data.data) {
        ...
        const nextActionDate = getHrNextActionDateValue(lead, profile);
        setCandidateDetails({
          ...lead,
          createdAt: lead.createdAt,
          leadOwner: lead.leadOwner?.name || lead.leadOwner || 'N/A',
          _course: { name: lead.applyingFor || 'N/A', batchName: 'N/A' },
          followupDate: nextActionDate,
          followup: { followupDate: nextActionDate },
        });
      } else {
  ...
  const fetchFollowupCounts = async (filters = filterData) => {
    try {
      ...
      const response = await axios.get(`${backendUrl}/college/hr/followupcounts?${queryParams}`, {
        headers: { 'x-auth': token }
      });
      console.log("response", response);
      if (response.data.success && response.data.data) {
  ...
  const fetchLeadHistory = async (profile = selectedProfile) => {
    ...
    try {
      ...
      if (response.data.success) {
        setLeadHistory(response.data.data?.logs || []);
      } else {
                                            ...
                                            <div className="scrollable-content">
                                              <div className="info-card">
                                                ...
                                                <div className="info-group">
                                                  ...
                                                  <div className="info-value">{formatLeadOwner(candidateDetails.leadOwner)}</div>
                                                </div>
                                                <div className="info-group">
                                                  ...
                                                  <div className="row">
                                                    ...
                                                    <div className="col-xl- col-3">
                                                      <div className="info-group">
                                                        ...
                                                        <div className="info-value">
                                                          {formatFollowupDateTime(getHrNextActionDateValue(candidateDetails, selectedProfile))}
                                                        </div>
