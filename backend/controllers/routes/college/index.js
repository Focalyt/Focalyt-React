...
const videoTimestampRoutes = require("./videoTimestamp");
...
/**
 * Owner filter = counsellor field only (not co-owner).
 * Counsellor filter = owner OR co-owner 1/2.
 * Without a person filter, team scope keeps registeredBy + owner + co-owners.
 */
function applyB2cLeadOwnershipMatch(baseMatch, { teamMemberIds, registeredByMe, ownerIds, counselorIds } = {}) {
	...
	const counselorObjectIds = toB2cObjectIdList(counselorIds);
	if (counselorObjectIds.length > 0) {
		delete baseMatch.counsellor;
		baseMatch.$or = [
			{ counsellor: { $in: counselorObjectIds } },
			{ leadCoOwner: { $in: counselorObjectIds } },
			{ leadCoOwner2: { $in: counselorObjectIds } }
		];
		return;
	}
...
/** Dept/project/course/center/batch/name selection widens Admin from own leads to all leads. */
...
async function resolveB2cOwnershipTeamMembers(user, counselorArray = [], options = {}) {
	...
	const isAdmin = user?.permissions?.permission_type === 'Admin';
	// Admin default: own/associated leads. All leads only if they picked a widening filter.
	if (isAdmin) {
...
router.route("/appliedCandidates").get(isCollege, async (req, res) => {
	try {
		const user = req.user;
		const college = (user.college && user.college._id)
			? user.college
		...
		const skip = (page - 1) * limit;
		const countOnly = req.query.countOnly === 'true' || req.query.countsOnly === 'true';
		...
		let teamMembers = hasPersonFilter
			? []
			: await resolveB2cOwnershipTeamMembers(user, counselorArray, {
				userSelectedWideningFilter,
		...
		if (followupStatus) {
			...
			const followupPersonIds = ownerArray.length > 0 ? ownerArray : counselorArray;
			if (followupPersonIds.length > 0) {
				followupMatch.$or = [
					{ counsellorId: { $in: toB2cObjectIdList(followupPersonIds) } },
					{ counsellorId: { $exists: false }, createdBy: { $in: toB2cObjectIdList(followupPersonIds) } },
					{ counsellorId: null, createdBy: { $in: toB2cObjectIdList(followupPersonIds) } },
				];
		...
		const processedResults = results.map(doc => {
			...
			return {
				...
				aiVoice: {
					...
					lastDisposition: aiVoiceDoc.lastDisposition || '',
					lastSummary: aiVoiceDoc.lastSummary || '',
...
function buildSimplifiedPipeline({ teamMemberIds, college, filters, pagination, forCount = false }) {
	...
	if (filters.centerArray && filters.centerArray.length > 0) {
		baseMatch._center = { $in: filters.centerArray.map(id => new mongoose.Types.ObjectId(id)) };
	}
...
router.route('/registrationCrmFilterCounts').get(isCollege, async (req, res) => {
	try {
		...
		applyB2cLeadOwnershipToMatch(basePipeline[0].$match, {
			teamMemberIds,
			ownerIds: ownerArray,
			counselorIds: ownerArray.length > 0 ? [] : counselorArray,
		});
		...
		const statusCounts = faceted.statusCounts || [];
		const approvalStatusCounts = faceted.approvalStatusCounts || [];
		const aiStatusCounts = faceted.aiStatusCounts || [];
...
const buildFollowupCounselorMatch = (user, counselorArray, ownerArray = []) => {
	const personIds = [
		...(Array.isArray(ownerArray) ? ownerArray : []),
		...(Array.isArray(counselorArray) ? counselorArray : []),
	].filter(Boolean);
	const uniqueIds = [...new Set(personIds.map((id) => String(id)))];
	if (uniqueIds.length > 0) {
		const ids = toB2cObjectIdList(uniqueIds);
		return {
			$or: [
				{ counsellorId: { $in: ids } },
	...
	// Admin and custom users both default to own follow-ups when no person filter.
	return {
		$or: [
...
router.get('/followupcounts', isCollege, async (req, res) => {
	try {
		...
		let baseMatch = {
			...buildFollowupCounselorMatch(user, counselorArray, ownerArray),
		...
		const needsFollowupJoins = projectsArray.length > 0
			|| verticalsArray.length > 0
		...
		if (needsFollowupJoins) {
			aggregate.push(
				...
				},
				{
					$lookup: {
						...
						foreignField: '_id',
						as: 'courseData',
						pipeline: [{ $project: { project: 1, vertical: 1 } }]
					}
				},
				{
					$unwind: {
			...
			if (courseArray.length > 0) {
				additionalMatches['appliedCourseId._course'] = { $in: courseArray.map(id => new mongoose.Types.ObjectId(id)) };
			}
			if (centerArray.length > 0) {
				additionalMatches['appliedCourseId._center'] = { $in: centerArray.map(id => new mongoose.Types.ObjectId(id)) };
			}
			if (batchArray.length > 0) {
				additionalMatches['appliedCourseId.batch'] = { $in: batchArray.map(id => new mongoose.Types.ObjectId(id)) };
			}
			if (createdFromDate || createdToDate) {
				additionalMatches['appliedCourseId.createdAt'] = {};
				if (createdFromDate) {
					additionalMatches['appliedCourseId.createdAt'].$gte = new Date(createdFromDate);
				}
				if (createdToDate) {
					const toDate = new Date(createdToDate);
					toDate.setHours(23, 59, 59, 999);
		...
		followupCounts.forEach(item => {
			const typeKey = item?._id?.type === 'visit' ? 'visit' : 'call';
			const statusKey = String(item?._id?.status || '').toLowerCase();
			const countVal = Number(item.count) || 0;
			if (statusKey === 'done' || statusKey === 'planned' || statusKey === 'missed') {
				byType[typeKey][statusKey] += countVal;
			}
			if (statusKey === 'done') {
				doneCount += countVal
			}
			if (statusKey === 'planned') {
				plannedCount += countVal
			}
			if (statusKey === 'missed') {
				missedCount += countVal
			}
		})
	...
	try {
		...
		if (req.query.countOnly === 'true' || req.query.countsOnly === 'true') {
			return res.status(200).json({
				...
				page,
				limit,
				pendingKycCount: crmFilterCounts.pendingKyc || 0,
				doneKycCount: crmFilterCounts.doneKyc || 0,
				totalCount: listTotalCount,
				totalPages: Math.ceil((listTotalCount || 0) / limit) || 0,
				data: [],
				crmFilterCounts
			});
		}
...
router.get("/leads/my-followups", isCollege, async (req, res) => {
	try {
		const user = req.user;
		let filter = {};
		const { fromDate, toDate, page = 1, limit = 10, followupStatus, projects, verticals, course, center, counselor, owner, name: searchName, filterBy } = req.query;
		const useActivityFilter = filterBy === 'activity';
