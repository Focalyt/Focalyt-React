...
router.post('/add-role',logUserActivity('role-added'), async (req, res) => {
  ...
  async (req, res) => {
	try {
		const { collegeId, designation, roleId, name, email, mobile, password, confirmPassword } = req.body;

		// 1. Validation
		if (!collegeId || !email || !mobile || !name || !password || !confirmPassword) {
			return res.status(400).json({ status: false, error: "All fields are required" });
		}

		if (password !== confirmPassword) {
			return res.status(400).json({ status: false, error: "Password and confirm password do not match" });
		}
